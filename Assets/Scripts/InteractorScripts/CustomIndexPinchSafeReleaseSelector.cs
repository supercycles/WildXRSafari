...
namespace Oculus.Interaction
{
    ...
    //We are using this script to determine when a player is pinching as they are looking at a grabbed butterfly
    //If they are pinching, we fill a progress bar, and when they release their pinch, if the bar is full, we select the highlighted part
    public class CustomIndexPinchSafeReleaseSelector : MonoBehaviour,
        ISelector, IActiveState
    {
        ...
        private void HandleHandUpdated()
        {
            ...
            //Ethan: if we are pinching, enable the "Hit Point Load" progress bar, and fill it
            if (isPinching)
            {
                Camera.main.GetComponent<HeadRay>().hitPointLoad.enabled = true;
                Camera.main.GetComponent<HeadRay>().hitPointLoad.fillAmount += .8f * Time.deltaTime;
            }
            ...
            if (_active && !isPinching && IsIndexExtended())
            {
                if (_selectOnRelease)
                {
                    ...
                    //Ethan: When we have released our pinch, if we did so when the progress bar was full, then "select" the part we were highlighting
                    if (Camera.main.GetComponent<HeadRay>().hitPointLoad.fillAmount == 1)
                    {
                        Camera.main.GetComponent<HeadRay>().pinched = true;
                        Camera.main.GetComponent<HeadRay>().hitPointLoad.fillAmount = 0;
                    }
                    Camera.main.GetComponent<HeadRay>().hitPointLoad.fillAmount = 0;
                }
                else
